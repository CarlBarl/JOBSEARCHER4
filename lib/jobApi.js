...
export async function isValidLogo(imageUrl) {
  ...
  try {
    // Instead of HEAD request, create an Image object and try to load the image
    ...
    return new Promise((resolve) => {
      ...
      // On successful load, clear timeout and resolve true
      img.onload = () => {
        clearTimeout(timeoutId);
        // Check if image is larger than 1x1 pixels (API returns 1x1 white pixel for missing logos)
      ...
      // On error, clear timeout and resolve false
      img.onerror = () => {
        clearTimeout(timeoutId);
        resolve(false);
      };
      ...
      // For browsers that cache the image and might not trigger onload
      if (img.complete) {
        clearTimeout(timeoutId);
...
export function getStudentSearchStrategies() {
  return [
    ...
    },
    {
      ...
      url: 'https://links.api.jobtechdev.se/joblinks?q=student%20stockholm',
      description: 'Searches for student jobs in Stockholm'
    }
  ];
}
...
export async function advancedStudentSearch({
  ...
  const searchParams = {
    q: keywords ? keywords : (location ? location : 'student'),
